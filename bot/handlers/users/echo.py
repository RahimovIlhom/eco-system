...
from keyboards.default import admin_menu, participant_menu
...
async def bot_echo(message: types.Message):
    if message.text:  # Xabar matni bo'sh emasligini tekshirish
        if message.text in ['🔙 Orqaga', '🔙 Назад']:
            lang = 'uz' if message.text == "🔙 Orqaga" else 'ru'
            await message.answer("Bosh manyu" if lang == 'uz' else "Главное меню", reply_markup=await participant_menu(lang))
        else:
            await message.answer(message.text)
    else:
        await message.answer("Xabar matni bo'sh!")
